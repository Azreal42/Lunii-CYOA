...
This is a general guard rail. All the projects specifics are in the readme.md. So first step will be to read the readme to get more precise context !

## Outline of mandatory steps when doing a task for the user
### Design a plan.md
Create a {id}-{feature}-plan.md that outline the complete plan, but also difficulties and traps of the task to perform.
It should somehow reflect the user prompt in an elaborate way, and it will serve as spec document.
It should be in folder "plans", and {id} is an incremental int that track all plans feature.
At the end, write a post mortem, and a list of modification in the plan.
Carefully find all existing code that might helps you. There is a lot of helpers in the code.
Note that you can get some insight from existing other plans to helps understand something.

### Internet
When you have trouble using third party library, look either for the third party documentation on internet, or on github if it's open source.

### Double check with a final code review of your changes
After the use of tools to ensure code quality, you also have to review your changes.
The goal is to :
1. Ensure the changes are minimals, and new code is not bloated.
2. Ensure the code is clean (no big files for instance)
3. It follows SOLID rules, and is professional level code
4. The code needs to have test coverage.

### What is forbidden for you
You can't commit and/or use git for any command that could change the remote.
You are only allowed to run git command to check history, and check local changes.
You are not allowed to run command that could need write rights on files that are not in workspace
(e.g. no write in the user folder for instance)
You can't perform action that could change the system behavior (network, or anything else).
If any of this is a mandatory step, stop the task and provide/display the next steps the user need to run for you to continue.

## Code rules
We're using 200 columns as standard, no need to have 80 columns code.
Keep it short, don't add exception for every case. Just one with a generic log containing the error is generally enough.
Do not add "section" comments made of separator lines (e.g. `# -----`) or similar banners.
Use OOP whenever possible and relevant.
- Keep `__init__.py` files empty (no exports or code, no `__all__`).

## Important General instructions, make sure to remember and follow :
When running python, since we use uv, always use `uv run <module>` to ensure we use the proper python version and environment.
You are running on windows, don't try to run linux/wsl commands.
When a package is missing, add it using `uv add <package_name>`, and if this is only for tests, `uv add <package_name> --extra tests`.
When a package is missing, don't try to bloat the code with checks and fallback on ImportError. Instead fix the venv !
As a general rule of thumb, don't bloat the code, keep it simple ! Stop the current task and ask the user more precision or some helps. The user is an expert at coding, he will review the code, he can guide you, don't think you're alone in this.

Before finalizing a task, always run multiple code quality tools (c.f below !)

When a file is at the center of the feature and you know it will be a major file you will modify, read by large chunk, in this case token count isn't really important.
We can use the feature of python 3.11 (so don't write legacy python code).

Don't try to write backward compatible code unless specifically asked to do so.

### Regenerating protobufs

- Always run the protoc from the repository root e.g. : `uv run python -m grpc_tools.protoc -I proto --python_out=src --grpc_python_out=src proto/news_processing.proto`
- After regeneration, verify that generated file has imports the message module as `import src.something as xxx`. If protoc strips the `src.` prefix, update the import manually to keep module resolution consistent.
...
1. **Always Use Type Hints**
   ...
   - Do not use `Optional[xxx]` use `xxx | None` pattern instead
Avoid the type hint `Any`, using it should be only last resort.
...
- Use Pandera schema whenever possible instead of pd.DataFrame typehint.
...
1. **Graceful Error Handling**
   - Use try/except but avoid overusing it (for instance don't wrap whole functions in try/except, and don't try/except just to log and re-raise)
   - Catch specific exceptions
   - Don't try/except something you can't recover from, just let the exception propagate
...
Before finalizing a task, run multiple code quality tools:
- Pyright: Static type checking
- Ruff: Fast Python linter
- Mypy: Static type checker

Always fix any issues reported by these tools before proceeding.
Remember to use `uv` to ensure you are using the venv when running those tools.
Run them by excluding the .venv
...
If the code needs refactoring, with the best practices
